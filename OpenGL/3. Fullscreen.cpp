...
int main()
{
	...
	if (Fullscreen)
	{
			...
			const GLFWvidmode* Modo = glfwGetVideoMode(MonitorPrimario);
			if(Modo!=NULL)
			{
				Ventana = glfwCreateWindow(Modo->width,Modo->height,"CONCHA",MonitorPrimario,NULL);
			}
	}
	else
	{
		Ventana  = glfwCreateWindow(ANCHO,ALTO,"Hello World!",nullptr,nullptr);
	}
